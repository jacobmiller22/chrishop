...
4. Local Verification: Run `pnpm run check`, `pnpm run build`, and automated tests. Post local verification comment on the issue.
...
- Monorepo verification evidence
...
When a worker reports completion, the Orchestrator MUST NOT take its word at face value. The Orchestrator conducts an **Adversarial Audit** across 5 criteria before accepting the deliverable:
...
- [ ] 5. **Monorepo Integrity**: Run `pnpm run check` at repository root to ensure zero cross-workspace TypeScript errors.
```

### Remediation Protocol
If an audit item fails:
1. Send a message to the worker subagent specifying the exact deficiency (e.g. *"Worktree was not reaped; please run `wt switch main && wt remove --reap ...`"* or *"Completion comment missing file breakdown"*).
...
- *"Orchestrate the next batch of stories"*
- *"Run story-orchestrator"*
- *"Find and execute the top shovel-ready stories"*
