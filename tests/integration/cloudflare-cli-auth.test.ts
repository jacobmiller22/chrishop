...
describe('Cloudflare Platform Provisioning & Wrangler CLI Configuration (Story 2.28)', () => {
  ...
  it('should verify wrangler.toml declares parameterizable routes for jacobmiller22.com', () => {
    ...
    assert.ok(
      content.includes('pattern = "chrishop.jacobmiller22.com/*"') &&
        content.includes('zone_name = "jacobmiller22.com"'),
  ...
  it('should verify D1, KV, and R2 resource declarations in wrangler.toml', () => {
    ...
    assert.match(
      content,
      /database_name\s*=\s*"chrishop-prod-db"/,
      'D1 database name must be chrishop-prod-db'
    );

    assert.ok(content.includes('[[kv_namespaces]]'), 'Must declare [[kv_namespaces]]');
    assert.match(
      content,
      /binding\s*=\s*"NEXT_CACHE_WORKERS_KV"/,
      'KV binding must be NEXT_CACHE_WORKERS_KV'
    );
    ...
    assert.match(
      content,
      /bucket_name\s*=\s*"chrishop-media-prod"/,
      'R2 bucket name must be chrishop-media-prod'
    );

    // Staging bindings
    assert.ok(
      content.includes('[[env.staging.d1_databases]]'),
      'Must declare [[env.staging.d1_databases]]'
    );
    assert.match(
      content,
      /database_name\s*=\s*"chrishop-staging-db"/,
      'Staging D1 database name must be chrishop-staging-db'
    );
    assert.ok(
      content.includes('[[env.staging.kv_namespaces]]'),
      'Must declare [[env.staging.kv_namespaces]]'
    );
    assert.ok(
      content.includes('[[env.staging.r2_buckets]]'),
      'Must declare [[env.staging.r2_buckets]]'
    );
    assert.match(
      content,
      /bucket_name\s*=\s*"chrishop-media-staging"/,
      'Staging R2 bucket name must be chrishop-media-staging'
    );

    // Preview bindings
    assert.ok(
      content.includes('[[env.preview.d1_databases]]'),
      'Must declare [[env.preview.d1_databases]]'
    );
    assert.ok(
      content.includes('[[env.preview.kv_namespaces]]'),
      'Must declare [[env.preview.kv_namespaces]]'
    );
    assert.ok(
      content.includes('[[env.preview.r2_buckets]]'),
      'Must declare [[env.preview.r2_buckets]]'
    );
  });
  ...
  it('should verify R2 CORS policies allow custom subdomains and localhost', () => {
    ...
    assert.ok(
      origins.includes('https://chrishop.jacobmiller22.com'),
      'Must allow https://chrishop.jacobmiller22.com'
    );
    assert.ok(
      origins.includes('https://shop.jacobmiller22.com'),
      'Must allow https://shop.jacobmiller22.com'
    );
  ...
  it('should verify wrangler CLI is operational and can generate environment types', () => {
    ...
    for (const env of ['staging', 'preview']) {
      const output = execSync(`pnpm exec wrangler types --env ${env}`, {
        cwd: rootDir,
        encoding: 'utf-8',
      });
      assert.ok(
  ...
  it('should validate API credentials and provide graceful diagnostics if unauthenticated', () => {
    ...
    if (hasToken && hasAccountId) {
      assert.ok(
        process.env.CLOUDFLARE_API_TOKEN!.length > 10,
        'CLOUDFLARE_API_TOKEN must have valid token length'
      );
      assert.ok(
        process.env.CLOUDFLARE_ACCOUNT_ID!.length > 10,
        'CLOUDFLARE_ACCOUNT_ID must have valid hex length'
      );
    } else {
      ...
      const whoamiOutput = execSync('pnpm exec wrangler whoami', {
        cwd: rootDir,
        encoding: 'utf-8',
      });
      assert.ok(
        whoamiOutput.includes('wrangler login') ||
          whoamiOutput.includes('Account Name') ||
          whoamiOutput.includes('Account ID'),
