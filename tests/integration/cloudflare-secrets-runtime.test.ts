...
describe('Cloudflare Workers Secrets vs GitHub Secrets Evaluation (Story 2.35)', () => {
  ...
  describe('1. @opennextjs/cloudflare Runtime Environment Propagation', () => {
    it('should simulate populateProcessEnv and verify edge secrets map to process.env without build-time baking', () => {
      ...
      assert.equal(
        simulatedProcessEnv.PAYLOAD_SECRET,
        'live-edge-payload-secret-at-least-32-chars-long'
      );
      assert.equal(simulatedProcessEnv.SHOPIFY_ADMIN_TOKEN, 'shpat_live_edge_admin_token');
      assert.equal(
        simulatedProcessEnv.DB,
        undefined,
        'Non-string bindings like D1 DB must not be assigned as string env'
      );
    ...
    it('should allow safe hermetic builds and tests when edge secrets are omitted (CI fallback mode)', () => {
      ...
      assert.ok(
        validated.PAYLOAD_SECRET.length >= 32,
        'Default PAYLOAD_SECRET meets 32 char minimum'
      );
  ...
  describe('2. Secret Hygiene & wrangler.toml Zero-Leakage Audit', () => {
    ...
    it('should confirm .env.example documents secrets with local .dev.vars instructions', () => {
      ...
      assert.ok(
        ...
        '.env.example must document SHOPIFY_ADMIN_TOKEN'
      );
      assert.ok(
        content.includes('SHOPIFY_STOREFRONT_TOKEN='),
        '.env.example must document SHOPIFY_STOREFRONT_TOKEN'
      );
      assert.ok(
        content.includes('SHOPIFY_WEBHOOK_SECRET='),
        '.env.example must document SHOPIFY_WEBHOOK_SECRET'
      );
    });
  });
  ...
  describe('3. CI/CD Pipeline Least-Privilege & Boundary Verification', () => {
    it('should verify deploy.yml only grants Cloudflare API credentials to GitHub Actions', () => {
      ...
      assert.ok(
        content.includes('secrets.CLOUDFLARE_API_TOKEN'),
        'deploy.yml must use CLOUDFLARE_API_TOKEN'
      );
      assert.ok(
        content.includes('secrets.CLOUDFLARE_ACCOUNT_ID'),
        'deploy.yml must use CLOUDFLARE_ACCOUNT_ID'
      );

      // Verify no application secrets are referenced in deploy.yml
      assert.ok(
        !content.includes('secrets.PAYLOAD_SECRET'),
        'deploy.yml must not require PAYLOAD_SECRET'
      );
      assert.ok(
        !content.includes('secrets.SHOPIFY_ADMIN_TOKEN'),
        'deploy.yml must not require SHOPIFY_ADMIN_TOKEN'
      );
      assert.ok(
        !content.includes('secrets.RESEND_API_KEY'),
        'deploy.yml must not require RESEND_API_KEY'
      );
    });
    ...
    it('should verify preview-deploy.yml adheres to least-privilege boundary', () => {
      ...
      assert.ok(
        ...
        'preview-deploy.yml must use CLOUDFLARE_API_TOKEN'
      );
      assert.ok(
        content.includes('CLOUDFLARE_ACCOUNT_ID'),
        'preview-deploy.yml must use CLOUDFLARE_ACCOUNT_ID'
      );
      assert.ok(
        !content.includes('secrets.PAYLOAD_SECRET'),
        'preview-deploy.yml must not require PAYLOAD_SECRET'
      );
      assert.ok(
        !content.includes('secrets.SHOPIFY_ADMIN_TOKEN'),
        'preview-deploy.yml must not require SHOPIFY_ADMIN_TOKEN'
      );
    });
  });
});
