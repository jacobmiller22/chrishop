...
export const AUDITED_TELEMETRY_SIGNALS: TelemetrySignal[] = [
  ...
  },
  {
    ...
    tier: 'storefront-app',
    type: 'metric',
    source: 'Browser Web Vitals API',
    destination: 'None (Uninstrumented)',
    state: 'missing',
    criticality: 'high',
    gapDetails: 'Zero client-side performance beaconing for LCP, INP, CLS, or TTFB from real shoppers on mobile devices during drop rushes.',
    followUpIssue: {
      id: 331,
  ...
  },
  {
    ...
    tier: 'storefront-app',
    type: 'event',
    source: 'Storefront UI Components (DropCountdown, PDP, Cart Drawer)',
    destination: 'None (Uninstrumented)',
    state: 'missing',
    criticality: 'critical',
    gapDetails: 'No unified telemetry pipeline tracking visitor progression through Drop Countdown -> PDP View -> Add to Cart -> Checkout Redirection -> Order Paid.',
    followUpIssue: {
      id: 333,
