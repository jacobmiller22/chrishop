...
export const AUDITED_TELEMETRY_SIGNALS: TelemetrySignal[] = [
  ...
  },
  {
    ...
    tier: 'webhook-pipeline',
    type: 'metric',
    source: 'Cloudflare Queue SHOPIFY_ORDERS_QUEUE / order-consumer.ts',
    destination: 'Workers Analytics Engine (WEBHOOK_ANALYTICS) / Edge Ingestion Telemetry & Health Probe',
    state: 'active',
    criticality: 'high',
    retentionOrCadence: 'Per batch processed / Rolling window in /api/health',
    followUpIssue: {
      id: 334,
  ...
  },
  {
    ...
    tier: 'webhook-pipeline',
    type: 'alert',
    source: 'Cloudflare Queue SHOPIFY_ORDERS_DLQ / order-consumer.ts',
    destination: 'Discord Webhook (OPS_ALERT_WEBHOOK_URL) / Cloudflare DLQ / Health Diagnostic Probes',
    state: 'active',
    criticality: 'critical',
    retentionOrCadence: 'Instantaneous Discord alert upon any message reaching DLQ',
    followUpIssue: {
      id: 334,
      title: 'Story 4.29: Shopify Webhook Queue Telemetry & Retry Backpressure Monitor',
    },
